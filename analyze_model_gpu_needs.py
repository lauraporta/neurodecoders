...
class GPUMemoryAnalyzer:
    ...
    def estimate_model_memory(
        ...
        optimizer_memory = (
            trainable_params * 8
        )  # Adam optimizer uses 2x param size

        # Batch memory
        batch_memory = (
            batch_size * input_shape[2] * input_shape[3] * self.dtype_size
        )

        # Rough activation memory estimate (very approximate)
        # This is a simplified estimate - in practice it varies greatly
        activation_memory = (
            batch_size * total_params * 0.1 * self.dtype_size
        )  # Rough estimate
        ...
        if TORCHINFO_AVAILABLE:
            try:
                info = torchinfo.summary(
                    model, input_size=input_shape, verbose=0, device="cpu"
                )
                activation_memory = (
                    info.total_mult_adds * self.dtype_size
    ...
    def find_latest_dataset(
        self, data_dir: str = "data", num_images: Optional[int] = None
    ) -> Optional[Dict]:
        ...
        try:
            ...
            file_info["file_size_mb"] = os.path.getsize(latest_file) / (
                1024 * 1024
            )
            ...
            if num_images is not None:
                ...
                file_info["total_memory_mb"] = (
                    file_info["per_sample_memory_bytes"] * num_images
                ) / (1024 * 1024)
    ...
    def _analyze_data_file(self, file_path: str) -> Dict:
        ...
        per_sample_memory = (
            array.nbytes / num_samples if num_samples > 0 else array.nbytes
        )
    ...
    def recommend_gpu(
        ...
        if (
            best_specs["vram_gb"] * 1024 >= required_memory_mb * 2
        ):  # If we have 2x memory
            gpus_needed = 1
            ...
            gpus_needed = max(
                ...
                int(
                    np.ceil(
                        required_memory_mb / (best_specs["vram_gb"] * 1024)
                    )
                ),
            )
    ...
    def _generate_suggestions(
        self, required_memory: float, available_memory: float
    ) -> List[str]:
        ...
        if utilization > 0.8:
            suggestions.append(
                "⚠️  High memory utilization - consider reducing batch size"
            )
        ...
        if utilization < 0.3:
            suggestions.append(
                "💡 You can increase batch size for better efficiency"
            )

        return suggestions

    def generate_slurm_command(
        self, gpu_recommendation: Dict, script_name: str = "train.py"
    ) -> str:
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze GPU memory needs for PyTorch models"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="128",
        help="Image size (e.g., 128 or 128x128)",
    )
    parser.add_argument(
        "--out-neurons", type=int, default=200, help="Number of output neurons"
    )
    parser.add_argument(
        "--num-images", type=int, help="Number of images in dataset (required)"
    )
    parser.add_argument(
        "--num-neurons",
        type=int,
        help="Number of neurons in dataset (required)",
    )
    parser.add_argument(
        "--dtype",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=["SimpleEncoder", "SimpleDecoder"],
    ...
    if "SimpleDecoder" in args.models:
        models_to_analyze["SimpleDecoder"] = SimpleDecoder(
            args.out_neurons,
    ...
    gpu_recommendation = analyzer.recommend_gpu(
        total_memory_mb, args.batch_size
    )
    ...
    if args.dtype == "float32":
        mixed_precision_recommendation = analyzer.recommend_gpu(
            total_memory_mb, args.batch_size, use_mixed_precision=True
        )
        if (
            mixed_precision_recommendation["fits_on_single_gpu"]
            and not gpu_recommendation["fits_on_single_gpu"]
        ):
