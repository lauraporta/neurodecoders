...
class ResNetEncoder(nn.Module):
    ...
    def __init__(self, out_neurons, resnet_type='resnet18', freeze_backbone=True):
        super().__init__()
...
class ResNetEncoderLightningModule(pl.LightningModule):
    ...
    def __init__(self, out_neurons: int, resnet_type='resnet18', 
...
def train_resnet_encoder(
    ...
    """
    from neurodecoders.encoder.encoder import NeuralDataModule  # Import from existing encoder
    ...
    class UnfreezeCallback(pl.Callback):
        def on_train_epoch_start(self, trainer, pl_module):
            if trainer.current_epoch == unfreeze_epoch and freeze_backbone:
                ...
                pl_module.model.unfreeze_backbone(num_layers=2)  # Unfreeze last 2 layers
    
    callbacks.append(UnfreezeCallback())
    
    # Setup logger
    logger = TensorBoardLogger("data/lightning_logs", name=f"resnet_encoder_{resnet_type}")
...
def main(dataset_to_load, resnet_type='resnet18'):
    ...
    from neurodecoders.encoder.encoder import load_latest_data, preprocess_data, visualize_data, plot_training_results, save_predictions
    ...
    # Save predictions
    save_predictions(model.model, images, firing_rates, data_file, 'data', dataset_to_load)
