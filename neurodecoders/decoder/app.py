...
def load_trained_model(model_path):
    ...
    try:
        ...
        if 'synthdata_dataset-' in model_name:
            dataset_name = model_name.replace('decoder_', '').replace('.pth', '')
        ...
        st.info(f"Loading model with {saved_in_neurons} input neurons and {image_size}x{image_size} output size")
        
        # Initialize model with the correct parameters from the saved model
        model = DecoderLightningModule(in_neurons=saved_in_neurons, image_size=image_size)
    ...
    if st.button("🚀 Start Training", type="primary"):
        with st.spinner("Training decoder model..."):
            ...
            def update_psnr_plot(train_psnr, val_psnr):
                if len(train_psnr) > 1:  # Only plot if we have at least 2 points
            ...
            def update_correlation_plot(train_correlation, val_correlation):
                if len(train_correlation) > 1:  # Only plot if we have at least 2 points
                    fig = plot_correlation_curves(train_correlation, val_correlation)
            ...
            try:
                ...
                with col1:
                    st.metric("Final Train Loss", f"{model.train_losses[-1]:.4f}")
                with col2:
                ...
                # Save predictions
                save_predictions(model, firing_rates, images, input_file, dataset_to_load=dataset_path)
                ...
                fig = plot_reconstructions(test_actuals[:10], test_predictions[:10])
                st.pyplot(fig)
                ...
                if model is not None:
                    ...
                    if 'synthdata_dataset-' in model_name:
                        ...
                        if os.path.exists(dataset_file):
                            ...
                            st.info(f"Using original training data: {dataset_name}")
                            
                            # Get some test images for visualization
                            if training_images.ndim == 4:
                                test_images = training_images[:10, 0] if training_images.shape[1] == 1 else training_images[:10, 0]
                            else:
                                test_images = training_images[:10]
                            
                            # Generate reconstructions using the original training data
                            with torch.no_grad():
                                test_firing_rates = torch.tensor(training_firing_rates[:10], dtype=torch.float32).to(device)
                                test_reconstructions = model(test_firing_rates).cpu().numpy()
                                
                                # Ensure proper shape - remove extra dimensions if present
                                if test_reconstructions.ndim == 4:
                                    test_reconstructions = test_reconstructions.squeeze(1)  # Remove channel dimension if present
                                elif test_reconstructions.ndim == 2:
                                    # If it's 2D, reshape to (N, H, W)
                                    test_reconstructions = test_reconstructions.reshape(len(test_reconstructions), test_images.shape[1], test_images.shape[2])
                                
                                # Resize reconstructions to match original image dimensions if needed
                                if test_reconstructions.shape[1:] != test_images.shape[1:]:
                                    resized_reconstructions = []
                                    for recon in test_reconstructions:
                                        resized = resize(recon, test_images.shape[1:], preserve_range=True)
                                        resized_reconstructions.append(resized)
                                    test_reconstructions = np.array(resized_reconstructions)
                        else:
                            st.error(f"Original training data not found: {dataset_file}")
                            st.stop()
                    else:
                        st.error("Could not determine original training data from model filename")
                        st.stop()
