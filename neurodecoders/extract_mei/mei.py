...
class MEIOptimizer:
    ...
    def __init__(self, device, encoder_model, target_neuron_idx=0, sta_patterns=None, rf_coords=None):
        self.device = device
    ...
    def get_expected_firing_rate(self, image, target_neuron_idx):
        ...
        if image.shape[-1] < 224:  # This is a patch
            ...
            full_image = torch.zeros(1, 224, 224, device=self.device, dtype=torch.float32)
            ...
            full_image[0, y:y+patch_size, x:x+patch_size] = image.squeeze()
        ...
        try:
            simulator = SimulateResponse(self.device, [single_image], self.sta_patterns, len(self.sta_patterns))
        ...
        expected_firing_rate = torch.tensor(firing_rates[0, target_neuron_idx], device=self.device, dtype=torch.float32)
    ...
    def initialize_random_image(self, sta_size=None, noise_std=0.1):
        ...
        random_patch = torch.randn(1, 1, sta_size, sta_size, device=self.device, dtype=torch.float32) * noise_std
    ...
    def optimize_image(self, initial_image, learning_rate=0.01, num_iterations=1000, 
                      regularization_weight=0.001, update_interval=50, callback=None,
                      patience=100, factor=0.5, min_lr=1e-6):
        """
        ...
        for iteration in range(num_iterations):
            ...
            expected_firing_rate = self.get_expected_firing_rate(image, self.target_neuron_idx)
            ...
            if callback and iteration % update_interval == 0:
                ...
                callback(iteration, bounded_image, total_loss.item(), 
                        predicted_firing_rate.item(), expected_firing_rate.item(),
                        optimizer.param_groups[0]['lr'])
        
        return image.detach(), loss_history, firing_rate_history, expected_firing_rate_history, lr_history
    ...
    def compare_with_sta(self, optimized_image, sta_pattern, neuron_idx):
        ...
        if mei_np.shape != sta_np.shape:
            sta_np = resize(sta_np, mei_np.shape, order=1, mode='reflect', anti_aliasing=True)
        ...
        ssim_score = ssim(mei_np, sta_np, data_range=2.0)  # data_range = max - min = 1 - (-1) = 2
    ...
    try:
        ...
        if 'model.fc.6.weight' in state_dict:
            ...
            if fc_keys:
                ...
                raise ValueError("Could not determine model architecture from saved weights")
...
def save_mei_results(optimized_image, loss_history, firing_rate_history, 
                    comparison_metrics, model_path, neuron_idx, output_dir='data'):
...
def plot_mei_optimization(optimized_image, sta_pattern, loss_history, firing_rate_history, 
                         comparison_metrics, neuron_idx):
    ...
    metrics_values = [comparison_metrics['correlation'], 
                     comparison_metrics['cosine_similarity'], 
                     comparison_metrics['ssim']]
    bars = axes[1, 2].bar(metrics_names, metrics_values, color=['skyblue', 'lightgreen', 'lightcoral'])
