...
from mei import (MEIOptimizer, load_encoder_model, load_synthetic_data, 
                generate_sta_patterns, save_mei_results, plot_mei_optimization)
...
def load_available_models():
    ...
    try:
        ...
        if not files:
            st.error("No encoder model files found in data/ directory. Please train an encoder first.")
            return {}
        ...
        for file_path in files:
            ...
            display_name = filename.replace('encoder_model_', '').replace('.pth', '')
...
def infer_dataset_from_model(model_path):
    ...
    try:
        ...
        if 'synthdata_dataset-' in model_filename:
            ...
            dataset_identifier = model_filename.split('synthdata_dataset-')[1].split('.pth')[0]
            ...
            if os.path.exists(data_dir):
                dataset_files = [f for f in os.listdir(data_dir) if f.endswith('.npz') and 'synthdata' in f]
                if dataset_files:
...
def get_dataset_info(dataset_path):
    ...
    try:
        ...
        mod_time = datetime.datetime.fromtimestamp(os.path.getmtime(dataset_path))
        
        # Extract info from filename
        filename = os.path.basename(dataset_path)
        if 'synthdata_dataset-' in filename:
            # Parse dataset info from filename
            dataset_info = filename.split('synthdata_dataset-')[1].split('.npz')[0]
...
def get_model_info(model_path):
    ...
    try:
        ...
        if 'synthdata_dataset-' in filename:
            # Parse dataset info from filename
            dataset_info = filename.split('synthdata_dataset-')[1].split('_n_neurons')[0]
            dataset_info = dataset_info.replace('_', ' ')
        else:
            dataset_info = "Unknown dataset"
...
def main():
    ...
    with st.sidebar.expander("Model Details", expanded=False):
        ...
        st.write(f"**Modified:** {model_info['modified'].strftime('%Y-%m-%d %H:%M')}")
    ...
    if selected_dataset_path is None:
        st.error("Could not automatically find the appropriate dataset for the selected model. Please ensure the dataset file exists.")
        st.stop()
    
    # Show dataset info
    dataset_info = get_dataset_info(selected_dataset_path)
    st.sidebar.info(f"**Auto-selected Dataset:** {dataset_info['dataset_info']}")
    ...
    try:
        ...
        if n_neurons is None:
            raise ValueError("Could not determine number of output neurons from model structure.")
    ...
    if st.sidebar.button("🚀 Run MEI Optimization"):
        with st.spinner("Running MEI optimization..."):
            try:
                ...
                images, firing_rates, sta_info = load_synthetic_data(selected_dataset_path)
                ...
                if sta_info:
                    ...
                    sta_pattern = sta_patterns[target_neuron]
                    rf_coords = np.random.randint(0, 224 - 11, size=(n_neurons, 2))
                
                # Create MEI optimizer with STA data
                mei_optimizer = MEIOptimizer(device, encoder_model, target_neuron, 
                                           sta_patterns=sta_patterns, rf_coords=rf_coords)
                
                # Get STA patch size for initialization
                sta_patch_size = sta_patterns.shape[1]  # Should be 11x11 or similar
                ...
                def update_visualization(iteration, current_patch, loss, predicted_fr, expected_fr, current_lr):
                    # Update progress bar
                    progress = (iteration + 1) / num_iterations
                    progress_bar.progress(progress)
                    
                    # Update progress text
                    progress_placeholder.text(f"Step {iteration + 1}/{num_iterations} - Loss: {loss:.4f} - Predicted FR: {predicted_fr:.2f} Hz - Expected FR: {expected_fr:.2f} Hz - LR: {current_lr:.2e}")
                    
                    # Update patch - clear previous and show new
                    image_placeholder.empty()
                    fig, ax = plt.subplots(1, 1, figsize=(6, 6))
                    im = ax.imshow(current_patch.squeeze().cpu().numpy(), cmap='gray', vmin=-1, vmax=1)
                    ...
                    pred_fr_metric.metric("Predicted FR", f"{predicted_fr:.2f} Hz")
                    exp_fr_metric.metric("Expected FR", f"{expected_fr:.2f} Hz")
                    ...
                    ax_loss.plot(loss_history_realtime, linewidth=2, color='blue')
                ...
                optimized_patch, loss_history, firing_rate_history, expected_firing_rate_history, lr_history = mei_optimizer.optimize_image(
                ...
                image_path, history_path = save_mei_results(
                    optimized_patch, loss_history, firing_rate_history,
                    comparison_metrics, selected_model_path, target_neuron
                )
                ...
                fig_results = plot_mei_optimization(
                    optimized_patch, sta_pattern, loss_history, firing_rate_history,
                    comparison_metrics, target_neuron
                )
                ...
                with col2:
                    ...
                    plt.plot(firing_rate_history, label='Predicted FR', linewidth=2)
                    plt.plot(expected_firing_rate_history, label='Expected FR', linewidth=2, linestyle='--')
                ...
                col1, col2, col3, col4, col5 = st.columns(5)
                with col1:
                    st.metric("Correlation", f"{comparison_metrics['correlation']:.3f}")
                with col2:
                    st.metric("Cosine Similarity", f"{comparison_metrics['cosine_similarity']:.3f}")
                with col3:
                    st.metric("SSIM", f"{comparison_metrics['ssim']:.3f}")
                with col4:
                    st.metric("Final Predicted FR", f"{firing_rate_history[-1]:.2f} Hz")
                with col5:
                    st.metric("Final Expected FR", f"{expected_firing_rate_history[-1]:.2f} Hz")
                
                st.success(f"✅ MEI optimization completed! Results saved to {image_path}")
