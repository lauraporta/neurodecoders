...
def create_top10_neuron_average_images(
    images, responses, n_neurons=None, n_top_images=10
):
    """
    Create average images for each neuron using only the top N images by
    firing rate.
    ...
    print(
        f"Creating top-{n_top_images} average images for "
        f"{n_neurons} neurons..."
    )
    ...
    for neuron_idx in range(n_neurons):
        ...
        # Weighted sum: (n_top_images, channels, height, width) *
        # (n_top_images, 1, 1, 1)
...
def create_all_images_neuron_average_images(images, responses, n_neurons=None):
    """
    Create average images for each neuron using ALL images with firing
    rates as weights.
...
def plot_correlation_analysis(analysis_results, save_path=None):
    """
    Plot correlation analysis between firing rates and average image
    statistics.
    """
    ...
    axes[0, 0].set_title(
        f"Max FR vs Avg Image Std\n"
        f"r={analysis_results['max_fr_vs_avg_std_corr']:.3f}, "
        f"p={analysis_results['max_fr_vs_avg_std_p']:.3e}"
    )
    ...
    axes[0, 1].set_title(
        f"Mean FR vs Avg Image Std\n"
        f"r={analysis_results['mean_fr_vs_avg_std_corr']:.3f}, "
        f"p={analysis_results['mean_fr_vs_avg_std_p']:.3e}"
    )
    ...
    axes[0, 2].set_title(
        f"Std FR vs Avg Image Std\n"
        f"r={analysis_results['std_fr_vs_avg_std_corr']:.3f}, "
        f"p={analysis_results['std_fr_vs_avg_std_p']:.3e}"
    )
...
def plot_high_vs_low_std_examples(
    average_images, responses, rf_coords, stas, n_examples=6, save_path=None
):
    """
    ...
    for i, (neuron_idx, std_type) in enumerate(
        zip(
        ...
        ax1.imshow(img_display, cmap="gray")
        ax1.set_title(
            f"{std_type} Std Neuron {neuron_idx}\n"
            f"Std: {avg_image_stds[neuron_idx]:.4f}"
        )
        ax1.axis("off")
        ...
        rect = Rectangle(
            ...
            rf_size,
            linewidth=2,
            edgecolor="red" if std_type == "High" else "blue",
            facecolor="none",
            alpha=0.8,
        )
        ax1.add_patch(rect)
