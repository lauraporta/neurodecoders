...
import torch
from create_simulated_neural_responses import (
    plot_neural_correlations,
    plot_response_heatmaps_all,
...
def main():
        ...
        model_layers = {
            ...
            "ResNet": {
                "layers": [
                    ...
                    13,
                    16,
                ],  # First conv layer of each block in ResNet18
                "names": [
                    "Conv1",
                    "Block1",
                    "Block2",
    ...
    # Add patch size input for pattern-based STAs
        ...
        if patch_size % 2 == 0:
            patch_size += 1
            st.sidebar.info(
                f"Patch size adjusted to {patch_size} (must be odd)"
            )
    ...
    if st.sidebar.button("Generate New Data"):
        with st.spinner("Generating neural responses..."):
            ...
            fig4 = plot_response_heatmaps_all(
                firing_rates, dot_products, adaptation_states
            )
            st.pyplot(fig4)

            # Create and display the histogram plot
            fig5 = plot_response_histograms(
                firing_rates, dot_products, adaptation_states
            )
            st.pyplot(fig5)
