...
def plot_sta_and_spikes(images, responses, dot_products, adaptation_states, stas, coords, n_plot_images=5, n_top_neurons=5):    
    ...
    for i in range(n_plot_images):
        ...
        ax_fr.scatter(range(n_top_neurons), sorted_responses[i], 
        ...
        ax_dp.scatter(range(n_top_neurons), sorted_dot_products[i], 
        ...
        ax_ad.scatter(range(n_top_neurons), sorted_adaptation[i], 
                     c=colors[:n_top_neurons], s=100, marker='s')
        ...
        ax_img2.axis('off')
        ax_img2.set_title(f"Image {image_sort_idx[i + n_plot_images]} (Max Response: {image_max_responses[image_sort_idx[i + n_plot_images]]:.2f})")
        
        # Plot firing rates for second set
        ax_fr2 = fig1.add_subplot(gs[i, 6])
        ax_fr2.scatter(range(n_top_neurons), sorted_responses[i + n_plot_images], 
                      c=colors[:n_top_neurons], s=100)
        ...
        ax_dp2.scatter(range(n_top_neurons), sorted_dot_products[i + n_plot_images], 
                      c=colors[:n_top_neurons], s=100, marker='x')
        ...
        ax_ad2.scatter(range(n_top_neurons), sorted_adaptation[i + n_plot_images], 
                      c=colors[:n_top_neurons], s=100, marker='s')
...
def plot_response_heatmaps(responses, dot_products, adaptation_states, n_plot_images=10, n_plot_neurons=10):
    """
...
def main():
    ...
    # Add histogram plot
    fig4 = plot_response_histograms(firing_rates, dot_products, adaptation_states)
